/* =========================================================
   CRIMSON — Project detail page script
   Self-contained: gallery render, lightbox, reveal-on-scroll,
   mobile menu, header scroll state, smart back navigation.
   ========================================================= */

/* ---------- The 13 T-shirt designs ---------- */
const shirtDesigns = Array.from({length:13}, (_,i)=>{
  const n = String(i+1).padStart(2,'0');
  return {
    src:`../../images/clothing/crimson-studio/crimson-studio-shirt-${n}.jpg`,
    alt:`Crimson Studio T-shirt design ${n}`,
    label:`T-Shirt Design ${n}`
  };
});
...
function showDesign(idx){
  currentDesign = (idx + shirtDesigns.length) % shirtDesigns.length;
  const d = shirtDesigns[currentDesign];
